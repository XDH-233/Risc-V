...
public:
    ...
    Wrapper_1(const char * name){
      time = 0;
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess(&(top.readReg1));
      signalAccess[1] = new CDataSignalAccess(&(top.readReg2));
      signalAccess[2] = new CDataSignalAccess(&(top.writeReg));
      signalAccess[3] = new SDataSignalAccess(&(top.writeData));
      signalAccess[4] = new SDataSignalAccess(&(top.readData1));
      signalAccess[5] = new SDataSignalAccess(&(top.readData2));
      signalAccess[6] = new CDataSignalAccess(&(top.RegWrite));
      ...
      top.trace(&tfp, 99);
      tfp.open((std::string("/home/xdh311/IdeaProjects/Risc-V/./simWorkspace/RF/") + name + ".vcd").c_str());
      #endif
    }
