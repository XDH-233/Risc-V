...
public:
    ...
    Wrapper_1(const char * name){
      time = 0;
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess(&(top.opCode));
      signalAccess[1] = new CDataSignalAccess(&(top.Branch));
      signalAccess[2] = new CDataSignalAccess(&(top.MemRead));
      signalAccess[3] = new CDataSignalAccess(&(top.MemtoReg));
      signalAccess[4] = new CDataSignalAccess(&(top.ALUop));
      signalAccess[5] = new CDataSignalAccess(&(top.MemWrite));
      signalAccess[6] = new CDataSignalAccess(&(top.ALUSrc));
      signalAccess[7] = new CDataSignalAccess(&(top.RegWrite));

      #ifdef TRACE
      Verilated::traceEverOn(true);
      top.trace(&tfp, 99);
      tfp.open((std::string("/home/xdh311/IdeaProjects/Risc-V/./simWorkspace/Control/") + name + ".vcd").c_str());
      #endif
    }
