...
public:
    ...
    Wrapper_1(const char * name){
      time = 0;
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess(&(top.address));
      signalAccess[1] = new SDataSignalAccess(&(top.writeData));
      signalAccess[2] = new SDataSignalAccess(&(top.readData));
      signalAccess[3] = new CDataSignalAccess(&(top.memRead));
      signalAccess[4] = new CDataSignalAccess(&(top.memWrite));
      ...
      top.trace(&tfp, 99);
      tfp.open((std::string("/home/xdh311/IdeaProjects/Risc-V/./simWorkspace/dataMem/") + name + ".vcd").c_str());
      #endif
    }
