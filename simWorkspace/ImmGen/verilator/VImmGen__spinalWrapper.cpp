...
public:
    ...
    Wrapper_1(const char * name){
      time = 0;
      waveEnabled = true;
      signalAccess[0] = new IDataSignalAccess(&(top.io_instruction));
      signalAccess[1] = new QDataSignalAccess(&(top.io_immGenOut));

      #ifdef TRACE
      Verilated::traceEverOn(true);
      top.trace(&tfp, 99);
      tfp.open((std::string("/home/xdh311/IdeaProjects/Risc-V/./simWorkspace/ImmGen/") + name + ".vcd").c_str());
      #endif
    }
