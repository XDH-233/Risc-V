...
public:
    ...
    Wrapper_1(const char * name){
      time = 0;
      waveEnabled = true;
      signalAccess[0] = new SDataSignalAccess(&(top.address));
      signalAccess[1] = new IDataSignalAccess(&(top.instruction));

      #ifdef TRACE
      Verilated::traceEverOn(true);
      top.trace(&tfp, 99);
      tfp.open((std::string("/home/xdh311/IdeaProjects/Risc-V/./simWorkspace/instMem/") + name + ".vcd").c_str());
      #endif
    }
