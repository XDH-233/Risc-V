...
public:
    ...
    Wrapper_1(const char * name){
      time = 0;
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess(&(top.ALUop));
      signalAccess[1] = new CDataSignalAccess(&(top.funct7));
      signalAccess[2] = new CDataSignalAccess(&(top.funct3));
      signalAccess[3] = new CDataSignalAccess(&(top.data1));
      signalAccess[4] = new CDataSignalAccess(&(top.data2));
      signalAccess[5] = new CDataSignalAccess(&(top.Zero));
      signalAccess[6] = new CDataSignalAccess(&(top.Result));

      #ifdef TRACE
      Verilated::traceEverOn(true);
      top.trace(&tfp, 99);
      tfp.open((std::string("/home/xdh311/IdeaProjects/Risc-V/./simWorkspace/ALU/") + name + ".vcd").c_str());
      #endif
    }
